...
def get_next_version(start_version, count):
    """Generate next versions based on the start version and count."""
    ...
    if label:  # If there's a label, increment label version
        for i in range(count):
            version_with_label = f"ver-{major}-{minor}-{patch}_{label}-{start_label_version + i}"
            versions.append(version_with_label)
    else:
        # Handle cases without a label by incrementing the appropriate version component
        for i in range(count):
            new_version = f"ver-{major}-{int(minor) + i}-0"  # Increment minor version
